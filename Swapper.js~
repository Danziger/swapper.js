(function(global) {
	...
	Swapper.prototype.setDefaultByIndex=function(defaultIndex) {
		if(typeof defaultIndex == "number" && this.pagesCount > 0)
			this.defaultIndex = Math.max( Math.min(defaultIndex, this.pagesCount-1), 0);
		else
